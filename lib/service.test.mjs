...
import { mintAcross, serviceBase, servicesFor, waitForPort, originFor } from "./service.mjs";
...
// databuddy's own sign-up answered 403 INVALID_ORIGIN when it was asked at 127.0.0.1 with no page
// behind it: nearly every auth library checks the Origin it is asked from.
test("a sign-in is asked from the page that service itself names", () => {
  assert.equal(originFor(3000, ["http://localhost:3000", "https://app.example.com"]), "http://localhost:3000");
  assert.equal(originFor(3000, ["https://dash.example.com:3000"]), "https://dash.example.com:3000");
  assert.equal(originFor(4321, ["http://localhost:3000"]), "http://localhost:4321");
  assert.equal(originFor(3000, []), "http://localhost:3000");
});
