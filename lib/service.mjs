...
export async function mintAcross({ recipes, root, appDir, onPath, appPort, portFor, mint }) {
  ...
  const rows = [];
  for (const group of groups) rows.push(...((await mint(group.recipes, (await portFor(group)) ?? appPort))?.identities ?? []));
