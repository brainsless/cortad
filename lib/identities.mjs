...
def gen(role):
    ...
    return {
        "username": "bl-%s-%s" % (slug(role), tag),
        "email": "bl-%s-%s@example.invalid" % (slug(role), tag),
        "password": secrets.token_urlsafe(18) + "A1!",
        "name": "Brainsless %s" % role,
    }
...
def jwt(recipe, env):
    ...
    now = int(time.time())
    # ponytail: subject 1, the first account of a restored copy; read a real id off the stand-in
    # database when a customer's users table does not start there.
    claims = {"sub": "1", "id": 1, "iat": now, "exp": now + TTL}
