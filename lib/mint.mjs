...
export function makeIdentities({ root, work, envFiles, say, keepSecret, appDir = () => root }) {
  ...
  async function mint(body, port) {
    ...
    const emulated = Boolean(Object.assign({}, ...[...envs().all].reverse()).FIREBASE_AUTH_EMULATOR_HOST);
    const forProgram = recipes.filter((r) => r?.kind !== "firebase" || emulated);
    const script = identityScript(forProgram, { envPath: envs().path, base: `http://127.0.0.1:${port}`, headers: body?.headers ?? {} }).split("/tmp/bl-identit").join(join(dir, "bl-identit"));
